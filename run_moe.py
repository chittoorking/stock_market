"""
Run Mixture of Experts pipeline on 6-month data.
5 domain LLM experts + 1 aggregator for signal selection.

Usage:
    python run_moe.py                        # Last 10 days
    python run_moe.py --start -3 --days 3    # Last 3 days
"""
...
API_KEY = "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"
...
from app.agents.graph_llms import risk_with_graph, executor_with_graph, judge_with_graph, monitor_with_graph
from app.agents.moe_scanner import moe_scanner, MoETrace
...
memory = MarketMemory()  # For record_trade logging only — NOT fed to experts

import argparse
_parser = argparse.ArgumentParser()
_parser.add_argument('--start', type=int, default=-10)
_parser.add_argument('--days', type=int, default=10)
_parser.add_argument('--save-traces', action='store_true', help='Save full expert traces')
...
print(f"MoE MODE | Running on {len(test_dates)} days: {test_dates[0]} to {test_dates[-1]}")
...
for date in test_dates:
    ...
    for scan_bar in [6, 15]:
        ...
        # Enrichment (signals get RED/GREEN/YELLOW flags)
        ...
        # ═══ MoE: 5 experts + aggregator ═══
        print(f"  [{date} bar {scan_bar}] {len(signals)} signals ({red_count} red-flagged) -> EXPERTS[", end="", flush=True)
        ranked, trace = moe_scanner(
            signals_text, sector_text, all_data, date, scan_bar, API_KEY,
            active_trades=trade_graph.get_all_active(),
        )
        llm_calls += 6  # 5 experts + 1 aggregator
        print("] ", end="")
        time.sleep(0.5)
        ...
        all_traces.append({
            ...
            "volume": trace.volume_expert[:500],
            "sector": trace.sector_expert[:500],
            "price": trace.price_expert[:500],
            "momentum": trace.momentum_expert[:500],
            "market": trace.market_expert[:500],
            "aggregator": trace.aggregator[:500],
        })

        if not ranked:
            print("MoE: no signals survived")
            continue
        ...
        if not approved:
            print(f"MoE ranked {len(ranked)}, all vetoed by risk")
            continue
        ...
        print(f"MoE ranked {len(ranked)}, approved {len(approved)}, opened {opened}")

    # ═══ Simulate positions (monitor uses one-shot, not debate) ═══
    for sym, node in list(trade_graph.get_all_active().items()):
        ...
        for j in range(entry_bar+1, len(sb)):
            ...
            # ═══ ONE-SHOT MONITOR every 6 bars (not debate — save LLM calls) ═══
...
print()
print('='*100)
print(f'MoE MODE | {len(test_dates)} DAYS | {llm_calls} LLM calls')
print('='*100)
...
# ═══ MoE ANALYSIS ═══
print(f'\n{"="*100}')
print(f'MoE EXPERT ANALYSIS | {len(all_traces)} scans')
print(f'{"="*100}')
...
print(f'Signals seen: {total_signals} | Survived MoE: {total_ranked} | Filter rate: {(1-total_ranked/max(total_signals,1))*100:.0f}%')

print(f'\n--- COMPARISON ---')
print(f'  Baseline debate (gpt-4o-mini):  23 trades, 57% WR, +3.016%')
print(f'  MoE experts (this run):         {n} trades, {wins/n:.0%} WR, {total:+.3f}%')

# Show expert traces
print(f'\n--- EXPERT TRACES (first 3 scans) ---')
for tr in all_traces[:3]:
    print(f'\n[{tr["date"]} bar {tr["bar"]}] {tr["signals"]} signals -> {tr["ranked_count"]} ranked')
    print(f'  VOL:  {tr["volume"][:150]}...')
    print(f'  SEC:  {tr["sector"][:150]}...')
    print(f'  PRC:  {tr["price"][:150]}...')
    print(f'  MOM:  {tr["momentum"][:150]}...')
    print(f'  MKT:  {tr["market"][:150]}...')
    print(f'  AGG:  {tr["aggregator"][:200]}...')

# Save traces
if _args.save_traces and all_traces:
    trace_file = Path(f'moe_traces_{test_dates[0]}_{test_dates[-1]}.json')
